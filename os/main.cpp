...
int FcComd(int); //新增fc命令
void FcCompare(FCB *,FCB *);

int ReplaceComd(int); //新增replace命令

void BatchComd(int);//新增batch命令

//在create、open、write、read、rewind或fseek操作后指向操作文件
void toCurFile(char *);
...
int main()
{
    ...
    // 读入文件分配表FAT
    char yn;
    ifstream ffi("FAT2008.txt",ios::in);//打开文件FAT2008.txt
    if (!ffi)
    {
    ...
    //读入磁盘块Disk[ ]信息
    ffi.open("Disk2008.dat",ios::binary|ios::in);
    if (!ffi)
    {
    ...
    ffi.open("UdTab2008.dat",ios::binary|ios::in);
    if (!ffi)
    {
...
void HelpComd()				//help命令，帮助信息(显示各命令格式)
{
    ...
    int num;
    if(comd[0][0]!='\0' && comd[1][0] == '\0'){
        cout<<"请输入 help [数字] 以查看命令内容。"<<endl;
        cout<<" 1. create    \t" << " 2. open     \t" << " 3. write     \t\n";
        cout<<" 4. read      \t" << " 5. close    \t" << " 6. del       \t\n";
        cout<<" 7. dir       \t" << " 8. cd       \t" << " 9. md        \t\n";
        cout<<"10. rd        \t" << "11. ren      \t" << "12. attrib    \t\n";
        cout<<"13. copy      \t" << "14. rewind   \t" << "15. fseek     \t\n";
        cout<<"16. block     \t" << "17. closeall \t" << "18. uof       \t\n";
        cout<<"19. undel     \t" << "20. exit     \t" << "21. prompt    \t\n";
        cout<<"22. fat       \t" << "23. check    \t" << "24.replace    \t\n";
        cout<<"25. batch       \t\n";
    }else if(strlen(comd[1])>0) {

        num = atoi(comd[1]); //转成int型
        ...
        switch(num) {
            ...
            case 2:
                ...
                break;
            case 3:
                cout<<"write <文件名> [<位置/app> [insert]] \n在指定位置写文件(有插入功能)。\n";
                break;
            case 4:
                cout<<"read <文件名> [<位置m> [<字节数n>]] \n读文件，从第m字节处读n个字节。\n";
                break;
            case 5:
                cout<<"close <文件名> \n关闭文件。\n";
                break;
            ...
            case 23:
                ...
                break;
            case 24:
                cout<<"replace <文件名> <目录名> \n以“文件名”指定的文件，取代“目录名”指定目录中的同名文件。\n";
                break;
            case 25:
                cout<<"batch batchfile [s]\n"
                      "batch batchfile 从磁盘文件batchfile逐行读入命令行执行，直到batchfile中所有命令行都执行完毕为止。\n"
                      "batch batchfile s 从模拟盘Disk[][]中读入批处理文件，并逐条执行命令";
                break;
            default:
                cout << "无效的帮助号码" << endl;
        }
...
int ProcessPath(char* path,char* &Name,int k,int n,char attrib)
{
    ...
    for (i=len-1;i>=0;i--)
        if (path[i]=='/')
            break;
    // 将路径中最后一个名字分离出来，并由引用参数 Name 带回
    Name=&path[i+1];		//取路径中最后一个名字
    if (i==-1)
    {// 如果没有找到 '/', 则表示路径中只有文件名而没有目录
        s=curpath.fblock;
        strcpy(temppath,curpath.cpath);
    }
    else
...
int MdComd(int k)		//md命令处理函数
{
    ...
    s=ProcessPath(comd[1],DirName,k,0,attrib); //if k==1
    ...
    {
        char attribCopy[256];
        sprintf(attribCopy, "|%s", comd[2]); // 把竖线添加到字符串前

        i = GetAttrib(attribCopy, attrib);
...
int CloseComd(int k)				//close命令的处理函数：关闭文件
{
    ...
    char attrib='\0';
    FCB *p;
    int len = strlen(curfile.cpath);
    if (k<1)
    {
        cout<<"\n命令中缺少文件名。已自动关闭当前操作文件。\n";
        strcpy(comd[1], curfile.cpath);
...
int FcComd(int k){
    ...
    short s,s1,s2;
    char *FileName1,*FileName2,attrib;
    FCB *fcbp1,*fcbp2,*fcbptemp;
    ...
    s1=ProcessPath(comd[1],FileName1,k,0,'\20');//取FileName所在目录的首块号
    if (s1<1)			//路径错误
        return s1;		//失败，返回
    s=FindFCB(FileName1,s1,attrib,fcbptemp);		//取FileName的首块号(查其存在性)
    if (s<0)
    {
        cout<<"\n要打开的第一个文件不存在。\n";
        return -1;
    ...
    s2=ProcessPath(comd[2],FileName2,k,0,'\20');//取FileName所在目录的首块号
    if (s2<1)			//路径错误
        return s2;		//失败，返回
    s=FindFCB(FileName2,s2,attrib,fcbptemp);		//取FileName的首块号(查其存在性)
    if (s<0)
    {
        cout<<"\n要打开的第二个文件不存在。\n";
        return -1;
    ...
    FcCompare(fcbp1,fcbp2);
}

void FcCompare(FCB *fcb1,FCB *fcb2){
    bool flag = true;

    short size1,size2,sizeMax;//文件大小
    size1 = fcb1->Fsize;
    size2 = fcb2->Fsize;
...
//    char *diff1 = new char[size1];//错误内容
//    char *diff2 = new char[size2];


    cout<<"文件1长度： "<<size1 <<" 地址： "<<addr1<<endl;
    cout<<"文件2长度： "<<size2 <<" 地址： "<<addr1<<endl;
...
//            if (Disk[addr1][i]!=Disk[addr2][i]){}

//可以使用file_to_buffer读取文件
    char *buffer1 = new char[size1];
    char *buffer2 = new char[size2];
    file_to_buffer(fcb1,buffer1);
    file_to_buffer(fcb2,buffer2);

    for (int i = 0; i < sizeMax; i++) {
        if(flag){
            if (buffer1[i] != buffer2[i]) {
                flag = false;
                cout << "文件在第" << i << "个字节不同" << endl;
            }
//            }else{
//                diffLoc++;
//            }?
...
int ReplaceComd(int k){
    ...
    short s,s1,s2;
    char *FileName1,attrib;
    char allFileName1[PATH_LEN],allFileName2[PATH_LEN];//完整路径名
    int size;
    FCB *fcbp1,*fcbp2;
    ...
    if (s1<1)			//路径错误
        return s1;		//失败，返回
    s=FindFCB(FileName1,s1,attrib,fcbp1);

    //strcpy(temppath,curpath.cpath);process函数执行就会copy，可以直接使用temp

    strcpy(allFileName1,temppath);//将路径复制
    int len1 = strlen(temppath);
    if (temppath[len1-1]!='/'){
        strcat(allFileName1,"/");
    }
    strcat(allFileName1,FileName1);
    cout<<"文件名1"<<allFileName1<<endl;

    if (s<0)
    {
        cout<<"\n要打开的第一个文件"<<allFileName1<<"不存在。\n";
        return -1;
    }

    int len2 = strlen(comd[2]);
    if (comd[2][len2-1]!='/'){
        strcat(comd[2],"/");
    }
    ...
    s2=ProcessPath(comd[2],FileName1,k,0,'\20');//取FileName所在目录的首块号
    if (s2<1)			//路径错误
        return s2;		//失败，返回
    s=FindFCB(FileName1,s2,attrib,fcbp2);		//取FileName的首块号(查其存在性)

    ::strcpy(allFileName2,temppath);
    len2 = strlen(temppath);
    if (temppath[len2-1]!='/'){
        strcat(allFileName2,"/");
    }
    strcat(allFileName2,FileName1);
    cout<<"文件名2"<<allFileName2<<endl;

    if (s<0)
    {
        cout<<"\n要打开的第二个文件"<<allFileName2<<"不存在。\n";
        return -1;
    }
    ...
    {
        cout << "\n文件" << allFileName2 << "是只读属性的文件。是否继续操作？（y/n）\n";
        char choice;
        cin >> choice;
    ...
    //隐藏/系统操作
    if ((int)fcbp2->Fattrib == 2 || (int)fcbp2->Fattrib == 4)
    {
        cout << "\n文件" << allFileName2 << "具有隐藏和系统属性的文件不能被取代。\n";
        return -1;
    }

    if (!strcmp(allFileName1, allFileName2))
    {
        cout << "\n文件" << allFileName1 << "不能自己取代自己。\n";
        return -1;
    }
    ...
    buffer_to_file(fcbp2,Buffer);//用1转化的buffer改变2指向的文件
    return 1;
}
...
void BatchComd(int k){
    ...
    char cmd[INPUT_LEN];			//命令行缓冲区
    if(k == 1){
        ifstream bfi(comd[1],ios::in);
        if(!bfi.is_open()) {
            cout << "文件" << comd[1] << "打开失败";
            return;
        }
//        else{
//            cout<<"wenjiandakai"<<endl;
//        }
        //cout<<"nihao"<<endl;
        while (!bfi.eof()){ //直接复制main函数内代码
            cout<<"\nC:";					//显示提示符(本系统总假定是C盘)
            if (dspath)
                cout<<curpath.cpath;
            cout<<">";
            bfi.getline(cmd,INPUT_LEN);
            cout<<cmd<<endl;
        ...
        if(!::strcmp(comd[2],"s")){
            short s,size;
            char attrib = '\0', * FileName;
            char allFileName[PATH_LEN];	//存放文件全路径名
            FCB* fcbp;

            s = ProcessPath(comd[1],FileName,k,0,'\020');
            if (s<1){
                cout<<"文件不存在"<<endl;
                return;
            }
            s = FindFCB(FileName,s,attrib,fcbp);
            strcpy(allFileName, temppath);

            int len = strlen(temppath);
            if (temppath[len - 1] != '/')
                strcat(allFileName, "/");
            strcat(allFileName, FileName);	//构造文件的全路径名
            ...
            size = fcbp->Fsize;
            char *Buffer = new char[size];		//分配动态内存空间
            ...
            //下面是while循环方案，同样也可以实现，并且在循环到换行符时不需要j-1

/*            int j = 0,l = 0;                //循环，直到Buffer结束
            while (j < size + 1) {			//输入命令，分析并执行命令
                cout << "\nC:";					//显示提示符(本系统总假定是C盘)
                if (dspath)
                    cout << curpath.cpath;
                cout << ">";
...
/////////////////////////////////////////////////////////////////
//当前操作文件
void toCurFile(char *path)
{
    strcpy(curfile.cpath, path);
    cout << "当前操作文件->" << curfile.cpath << endl;
}